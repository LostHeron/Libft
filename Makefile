...
# Just an exemple of how to solve for make / make bonus rules asked in libft subject :
#
#
# NAME := libft.a
# CC := cc
...
# # following line beginning with two ## is to know wether last compilation was with bonus or not 
# # make_with_bonus = 1 means archive exist and made without bonus
# # make_with_bonus = 2 means archive exists and made with bonus
# # initial value set to 1 but should not matter
# MAKE_WITH_BONUS=2
#
# .PHONY: all clean fclean re bonus
# 
# all: 
# 	if [ $(MAKE_WITH_BONUS) -ne 1 ] ; then echo coucou ; sed -i 's/MAKE_WITH_BONUS=2/MAKE_WITH_BONUS=2/' Makefile; rm -f $(NAME); fi 
# 	$(MAKE) $(NAME) VAR_OBJECTS="${OBJECTS}"
# 
# bonus:
# 	if [ $(MAKE_WITH_BONUS) -ne 2 ] ; then echo coucou ; sed -i 's/MAKE_WITH_BONUS=2/MAKE_WITH_BONUS=2/' Makefile; rm -f $(NAME); fi 
# 	$(MAKE) $(NAME) VAR_OBJECTS="${OBJECTS} ${OBJECTS_BONUS}"
# 
# $(NAME): $(VAR_OBJECTS) 
# 	ar rcs $(NAME) $^
# 
